...
# Mol - main class the whole gel

# nodes - with positions  are created through Lattice
# Nodes - are Atom class
# Monomers - Class Atom
# They arey also bonded with a class Bond
# Counter ions (Cions) - Atom
...
class Mol():
    ...
    def create_cions(self):
        ...
        self.ncions = int(abs(self.total_charge))
        # n atoms don't work
        ...
        if self.ionization_every_left < self.ionization_every_right:
            # moving fraction to left
            # fraction of atoms is always (nr/nl - 1)/(nr/nl+1)
            print( "moving atoms to the left")
            _coeff = float(self.ionization_every_left)/self.ionization_every_right
            _fraction = (1. - _coeff)/(1. + _coeff)
            _natoms_to_move = _fraction * _N / 2.
            right_indices = np.where(R[:,0] > _L / 2.)[0]
            np.random.shuffle(right_indices)
            R[right_indices[:int(_natoms_to_move)],0] -= _L / 2.
            # print( "moving this many counter ions", _coeff, "")
            print( "ioniz coeff", _coeff)
            print( "moving this fraction atoms" , _fraction)
        elif self.ionization_every_left > self.ionization_every_right:
            # moving fraction to right
            print( "moving atoms to the right")
            _coeff = float(self.ionization_every_right)/self.ionization_every_left
            _fraction = (1. - _coeff) / (1 + _coeff)
            _natoms_to_move = _fraction * _N / 2.
            left_indices = np.where(R[:,0] < _L / 2.)[0]
            np.random.shuffle(left_indices)
            R[left_indices[:int(_natoms_to_move)],0] += _L/2.
        else:
            print( "both sides are ionized equally no moving")
