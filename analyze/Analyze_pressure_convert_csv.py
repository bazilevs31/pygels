...
def analyze_pressure_profile(inFileName):
    ...
    x = np.zeros(nBins, dtype=[('Id', np.float32), ('Coord1', np.float32), ('Ncount', np.float32),  ('NumDensity', np.float32), ('Pressure', np.float32)])
...
def main():
    # directory = 'profiles'
    for fname in glob.glob('/Users/bazilevs/Work/cedar/17-12-08/rerun_profiles/press*.txt'):
        print fname
        # analyze_Pressure_profile("energyalltot_sc1844_nm100_l50_fl02_fr05_t03_ljeq1_coul_eq_coulsim_press.txt")
        if os.path.isfile(fname):
            print "file exists"
            analyze_pressure_profile(fname)
