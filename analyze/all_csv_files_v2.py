...
def plot_local(x, y, y_divide_array_1, y_divide_array_2, ax, plot_details, plot_pure, plot_line=True, **kwargs):
    ...
    if plot_pure:
        ...
        # plotting calculated line for pressure potential
        Ngel  = 87120 + 288
        # y1 = log(c2/c1)
        # c2  = c1*exp(y1)
        yy = y/(y_divide_array_1 - y_divide_array_2)
        N1 = 0.5*Ngel*(0.1)
        N2 = 0.5*Ngel*(0.1*np.exp(y_divide_array_1))
        ax.plot(x, yy/(N2-N1)/1e-5, c+'--')
        # ax.plot(x, x*.25, 'k--')
...
def get_arrays_from_file(**kwargs):
    ...
    for temp_name in temp_name_array:
        ...
        if os.path.exists(f+'.csv') and ('fl01_fr02_t01' not in f):
            ...
            c1_array.append(concentration_min)
    ...
    if os.path.exists(csvfile):
        ...
        q1_array = np.array(df4['q1'])[tt3]
        q2_array = np.array(df4['q2'])[tt3]
    ...
    return np.array(temp_array), np.array(phi3_array_pure), np.array(conc_divide_array), np.array(f_divide_array), np.array(q1_q2_param_array), np.array(press_array), np.array(work_array),np.array(pzz_array), np.array(c1_array),np.array(c2_array),np.array(q1_array),np.array(q2_array),  fillstyle
...
def main(args):
    ...
    for myfile in params_dict['myfiles_list']:
        ...
        temp_array, phi3_array_pure, conc_divide_array, f_divide_array, q1_q2_param_array, press_array,work_array, pzz_array, c1_array, c2_array, q1_array, q2_array, fillstyle = get_arrays_from_file(**params_dict)

        # out_pd = pd.DataFrame()
        out_pd = pd.DataFrame(np.column_stack([temp_array,press_array, phi3_array_pure, conc_divide_array, q1_q2_param_array, c2_array, c1_array, q1_array, q2_array]),columns=['T','deltaP', 'phi', 'log_c2_c1', 'log_q2_q1', 'c2', 'c1', 'q1', 'q2'])
        # out_pd = pd.DataFrame(np.column_stack([temp_array,press_array, phi3_array_pure, conc_divide_array, q1_q2_param_array]),
                               # )
        out_pd.to_csv('result_{0[myfile]}.csv'.format(params_dict))
                ...
                if my_axes_name == my_param_name:
                    ...
                    if params_dict[my_param_name]:
                        if my_param_name == 'f':
                            ...
                            plot_local(press_array*200*200*1000/p_scale_factor, phi3_array_pure, conc_divide_array, q1_q2_param_array, ax_dict[my_param_name], plot_details=1,plot_pure=True, plot_line=False,color=c,filename=myfile,fillstyle=fillstyle)
